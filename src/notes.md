# Notes from C++ course
Do while loops faster than for
break; can be used to break out of loops (but not ifs)
continue skips what is below, goes back to top of loop
goto (bad  practice) skips to label:

## Arrays
int myarr [ 5] {10,22,33,22,3}  
static int myarr [ 5] // initializes at 0 can change?

### Folded arrays
using x[i][j] much less efficient than doing the calculation for index pointing

## Miscellaneous
inline type name(type)
