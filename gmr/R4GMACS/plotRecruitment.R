...
#' @export
plot.Recruitment <- function(M){
  mdf <- .get_recruitment_df(M)
  p <- ggplot(mdf,aes(x=(year),y=exp(log_rec),fill=Model,group=Model))
  p <- p + geom_bar(stat = "identity", alpha=0.75,position="dodge")
  p <- p + geom_linerange(aes((year),exp(log_rec),ymax=ub,ymin=lb),width=0.1
                          ,alpha=0.5,position = position_dodge(width = 0.9))
  p <- p + labs(x="Year",y="Recruitment")  #+ylim(c(0,500))
...
#' @author Cole Monnahan and Kelli Johnson
#' @export
