...
PARAMETER_SECTION
 ...
 init_bounded_dev_vector log_New_dev(2,nyrs,-5.0,4.0,ph_log_New_dev)
...
FUNCTION run_pop_dynamics
 ...
 for(t=1;t<=nyrs;t++)
 {
   ...
   // Note that ALL juveniles are discarded
   S=mfexp(-lag_pf(t)*MM(t));
   D(1) = (1.0-mfexp(-s_pf(1,t)*Fpf(t)*hm(1)));
   D(2) = (1.0-mfexp(-s_pf(2,t)*Fpf(t)*hm(1)));
   D(3) = (1.0-mfexp(-Fpf(t)*(1.0-dstr(t)+dstr(t)*hm(1))));

   // Calculate fishery removals (note pulse fishery)
   R_pf(t,1)=N(t,1)*S*D(1);
   R_pf(t,2)=N(t,2)*S*D(2);
   R_pf(t,3)=N(t,3)*S*D(3);                            
