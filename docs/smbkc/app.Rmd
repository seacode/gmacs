---
title: "Appendix to Gmacs SMBKC Stock Assessment"
output:
  pdf_document:
    highlight: zenburn
    toc: yes
  html_document:
    theme: flatly
    toc: yes
  word_document: default
bibliography: 
---

\newpage\clearpage

## The base model data file:

```{r eval = TRUE, include=TRUE,echo=FALSE }
opts_chunk$set(fig.width = 12, fig.height = 8, echo = FALSE, warning = FALSE, message = FALSE)
ts <- readLines("../../examples/smbkc2/model_1/sm15.dat")
for (i in 1:length(ts)) cat(ts[i], "\n")
```

## The base model control file:

```{r eval = TRUE, include=TRUE,echo=FALSE }
opts_chunk$set(fig.width = 12, fig.height = 8, echo = FALSE, warning = FALSE, message = FALSE)
ts <- readLines("../../examples/smbkc2/model_1/sm15.ctl")
for (i in 1:length(ts)) cat(ts[i], "\n")
```

\newpage\clearpage

## The selex model control file:

```{r eval = TRUE, include=TRUE,echo=FALSE }
opts_chunk$set(fig.width = 12, fig.height = 8, echo = FALSE, warning = FALSE, message = FALSE)
ts <- readLines("../../examples/smbkc2/model_2/sm15.ctl")
for (i in 1:length(ts)) cat(ts[i], "\n")
```

\newpage\clearpage

## The add CV model control file:

```{r eval = TRUE, include=TRUE,echo=FALSE }
opts_chunk$set(fig.width = 12, fig.height = 8, echo = FALSE, warning = FALSE, message = FALSE)
ts <- readLines("../../examples/smbkc2/model_3/sm15.ctl")
for (i in 1:length(ts)) cat(ts[i], "\n")
```

\newpage\clearpage

## The no $M_{1998}$ model control file:

```{r eval = TRUE, include=TRUE,echo=FALSE }
opts_chunk$set(fig.width = 12, fig.height = 8, echo = FALSE, warning = FALSE, message = FALSE)
ts <- readLines("../../examples/smbkc2/model_4/sm15.ctl")
for (i in 1:length(ts)) cat(ts[i], "\n")
```
