...
Response: The error in the 2015 was fixed and this model was run again. Comparisons between the Gmacs models and the 2015 model are presented throughout this document.
...
df <- dplyr::filter(data.frame(M[[2]]$dCatchData_out), V2 == 2, V3 %in% c(2,3), V5 > 0) %>%
...
6. **Gmacs force**: natural mortality ($M$) is fixed at 0.18 $\text{yr}^{-1}$ during all years and the directed pot, NMFS trawl survey and ADF&G pot survey selectivities for stage-1 and stage-2 crab are estimated. These selectivities are bounded so that they cannot be greater than 1. The Francis iterative re-weighting method is also used but the NMFS trawl survey is up-weighted by $\lambda^\text{NMFS}=$ `r M[[6]]$cpue_lambda[1]` and the ADF&G pot survey is up-weighted by $\lambda^\text{ADFG}=$ `r M[[6]]$cpue_lambda[2]`.
...
Results for all Gmacs scenarios are provided here with comparisons to the 2015 model. We recommend scenario XXX to be used for the overfishing determination in 2016, based on the fit to the data and the plausibility of parameter estimates.

### a. Effective sample sizes and weighting factors.

Observed and estimated effective sample sizes are compared in Table \ref{tab:effn}. Effective sample sizes are also shown on size-frequency plots (Figures \ref{fig:sc_pot}, \ref{fig:sc_pot_discarded}, and \ref{fig:sc_trawl_discarded}). Data weighting factors, SDNRs, and MARs are presented in Table \ref{tab:data_weighting}.
...
SD[j] <- SD[j] * 1000
Parameter <- c("Natural mortality deviation in 1998/99 $\\delta^M_{1998}$","$\\log (\\bar{R})$","$\\log (N_1)$","$\\log (N_2)$","$\\log (N_3)$","$\\log (\\bar{F}_\\text{pot})$","$\\log (\\bar{F}_\\text{trawl bycatch})$","$\\log (\\bar{F}_\\text{fixed bycatch})$","$F_\\text{OFL}$","OFL")
...
Estimate[j] <- Estimate[j] * 1000
SD[j] <- SD[j] * 1000
Parameter <- c("Natural mortality deviation in 1998/99 $\\delta^M_{1998}$","$\\log (\\bar{R})$","$\\log (N_1)$","$\\log (N_2)$","$\\log (N_3)$",
...
```{r recruitment, fig.cap = "Comparisons of estimated recruitment time series during 1979-2016 in each of the scenarios. The solid horizontal lines in the background represent the estimate of $\\bar{R}$ in each model scenario.\\label{fig:recruitment}"}
plot_recruitment(M)
```
...
\begin{align}
    F_{t,y}^tex{df} &= \bar{F}^tex{df} + \delta^tex{df}_{t,y}, \notag\\
    F_{t,y}^tex{tb} &= \bar{F}^tex{tb} + \delta^tex{tb}_{t,y}, \notag\\
    F_{t,y}^tex{fb} &= \bar{F}^tex{fb} + \delta^tex{fb}_{t,y},
\end{align}
where $\delta^tex{df}_{t,y}$, $\delta^tex{tb}_{t,y}$, and $\delta^tex{fb}_{t,y}$ are the fishing mortality deviations for each of the fisheries, each season $t$ during each year $y$, $\bar{F}^tex{df}$, $\bar{F}^tex{tb}$, and $\bar{F}^tex{fb}$ are the average fishing mortalities for each fishery. Also add selectivity, retension, discard mortality in here. Also catch, CPUE.
...
Estimated parameters are listed in Table \ref{tab:bounds_pars} and include an estimated parameter for natural mortality ($M$) in 1998/99 assuming an anomalous mortality event in that year, as hypothesized by Zheng and Kruse (2002), with natural mortality otherwise fixed at 0.18 $\text{yr}^{-1}$.
\begin{table}[ht]
\centering
...
\begin{tabular}{lrrrlr}
  ...
  Stage-1 initial numbers $\log (N_1)$ & 5 & 14.5 & 20 & Uniform(5,20) & 1 \\ 
  Stage-2 initial numbers $\log (N_2)$ & 5 & 14.0 & 20 & Uniform(5,20) & 1 \\ 
  Stage-3 initial numbers $\log (N_3)$ & 5 & 13.5 & 20 & Uniform(5,20) & 1 \\ 
  ...
  Natural mortality deviation during 1998 $\delta^M_{1998}$ & -3 & 0.0 & 3 & Normal($\delta^M_{1997}$, $\sigma^2_M$) & 4 \\ 
