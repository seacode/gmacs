...
# The model specs
.MODELDIR = c("../../examples/smbkc/", "../../examples/smbkc2/model_1/", "../../examples/smbkc2/model_2/", "../../examples/smbkc2/model_3/", "../../examples/smbkc2/model_4/")
...
tab <- xtable(df, digits=0, caption = "Population abundances (N) by crab stage in millions of crab, mature male biomasses at survey (MMB) in millions of pounds on 15 February for scenario 1. All abundances are at time of survey.", label = "tab:pop_abundance")
...
A <- M; A[[jj]] <- NULL
plot_molt_prob(A, xlab = "Carapace length (mm)")
```
...
A <- M; A[[jj]] <- NULL; A[["Gmacs selex"]] <- NULL; A[["Gmacs CV"]] <- NULL; A[["Gmacs M"]] <- NULL
plot_growth_inc(A)
```

```{r growth_trans, fig.cap = "Growth transitions.\\label{fig:growth_trans}"}
A <- M; A[[jj]] <- NULL
plot_growth_transition(A, xlab = "Carapace length (mm)")
```
...
plot_size_transition(M, xlab = "Carapace length after transition (mm)")
```
...
```{r natural_mortality, fig.cap = "Time-varying natural mortality ($M_t$). Specified pulse period occurs in 1998. \\label{fig:M_t}"}
...
Parameter <- c("$R_0$","$\\bar{R}$","$\\log (N_1)$","$\\log (N_2)$","$\\log (N_3)$","$q_{pot}$")
Estimate <- M[[2]]$fit$est[i]
Estimate[1:2] <- exp(Estimate[1:2])
df <- data.frame(Parameter, Estimate, Estimate, Estimate, Estimate, Estimate, Estimate, Estimate, Estimate)
names(df) <- c("Parameter","Estimate","LB","UB","Initial value","Prior type","p1","p2","Phaze")
...
x <- M[[2]]$fit
i <- c(grep("theta", x$names), grep("survey_q", x$names))
Parameter <- x$names[i]
Estimate <- x$est[i]
SD <- x$std[i]
