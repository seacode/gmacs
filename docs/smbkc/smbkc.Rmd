...
\begin{align}
  \sigma_{t,y} &= \sqrt{\log \left( 1 + cv_{t,y}^2 \right)}\\
  \delta_{t,y} &= \mathcal{N} (0, \sigma_{t,y}^2)
\end{align}
where $\delta_{t,y}$ is the residual catch. The relative abudance data is also assumed to be lognormally distributed
\begin{align}
  \sigma_{t,y} &= \frac{1}{\lambda} \sqrt{\log \left( 1 + cv_{t,y}^2 \right)}\\
  \delta &= \log \left( obs / pre \right) / \sigma + 0.5 \sigma
\end{align}
and the likelihood is
\begin{equation}
  \sum \log (\delta) + \sum 0.5 \sigma^2
\end{equation}
