...
for (j=6;j<=13;j++) 
  sel_ret0(j) = ret_a  + ret_b*length_bins(j);
...
for (j=6;j<=16;j++) 
  if (sel_ret0(j) < 0.0) 
    sel_ret0(j) = 0.0;
for (j=6;j<=14;j++) 
  sel_ret(j) = ret_a  + ret_b*length_bins(j);
...
FUNCTION get_mortality
 ...
 t0 = 0.0;
  ...
  for (i=yrs_fix(1); i<=endyr; i++) 
    fmortf(i)=mfexp(log_avg_fmortf+fmortf_dev(i));
...
ofstream report1("refp75162b.out");
...
ofstream report2("prj75162b.out");
