...
// !!cout<<nsamples_fish_discm<<endl;
  ...
  init_number nobs_fix                       //number of years of fixed gear bycatch length data
// !!cout<<nsamples_trawl<<endl;
  init_vector yrs_fix(1,nobs_fix)          //years when have fixed gear bycatch data
  init_matrix nsamples_fix(1,2,1,nobs_fix) //weight for fixed gear bycatch data
// !!cout<<nsamples_fix<<endl;
...
//!!cout<<obs_p_fish_discm(1)<<endl;
  init_3darray obs_p_trawl(1,2,1,nobs_trawl,1,nlenm)        //female and male trawl bycatch data
  init_3darray obs_p_fix(1,2,1,nobs_fix,1,nlenm)        //female and male trawl bycatch data
//!!cout<<obs_p_fix(1,1)<<endl;
...
//  !!cout<<catch_numbers<<endl;
  ...
  init_vector catch_fix(styr,endyr)  //fixed gear bycatch numbers sex combined need to apply mort 50%
...
!!ad_comm::change_datafile_name("rksize16b.dat");
  ...
  init_matrix obs_s_fix(1,2,1,nobs_fix)           //observed fixed bycatch length data sample sizes for females and males
  init_matrix obs_s_tc(1,2,1,nobs_tc)               //observed Tanner crab fishery bycatch length data sample sizes for females and males
//  !!cout<<obs_s_tc<<endl;

!!ad_comm::change_datafile_name("rk7516b.ctl");
  ...
  init_number m_fix         //fraction of trawl discards that die(.8)
...
//  !!cout<<"end of data reading"<<endl;
 ...
 LOCAL_CALCS
  // cout<<"to local calcs"<<endl;
   ...
   catch_fix=catch_fix*1000000;               //fixed gear bycatch numbers read in are millions of crab
...
//parameters to be estimated are all ones that begin with init_ and have a positive phase, negative phase means are fixed.
  ...
  init_bounded_number Mm(0.0184,1.0,6)
...
//      init_bounded_number a_bsfrf(0.0,0.5,7)
    init_bounded_number a_bsfrf(0.0,0.04,6)
...
//  init_bounded_dev_vector fish_disc_sel50_dev_f(1,nobs_fish_discf,-30,30,phase_logistic_sel+1)
  ...
  init_bounded_number fish_disc_slope_fi(.01,.2,phase_logistic_sel)
  init_bounded_number log_fish_disc_sel50_fi(4.5,5.4,phase_logistic_sel)
...
//    init_bounded_number qm2(0.75,1.0,survsel_phase+3)
   ...
   //  init_bounded_number qm2y(-6.0,50.31,6)
     init_bounded_number qm2y(-4.5,5.50,6)
  number qm2
...
//  init_bounded_dev_vector fmortt_dev(styr,endyr,-10,10.0,1)
  ...
  init_bounded_number log_avg_fmortf(-8.5,-0.5,1)
  init_bounded_dev_vector fmortf_dev(2009,endyr,-10,10.0,1)
...
//high grading
  ...
  matrix sel_fix_f(styr,endyr,1,nlenm)
  matrix sel_fix_m(styr,endyr,1,nlenm)
...
//  matrix sel_srv0(1,2,1,nlenm)
  ...
  matrix totn_fix(1,2,styr,endyr)
  ...
  3darray pred_p_fix(1,2,styr,endyr,1,nlenm)
  ...
  vector pred_catch_fix(styr,endyr)
  ...
  3darray Fdiscfi(1,2,styr,endyr,1,nlenm)
...
//  vector mbio215(styr,endyr)
  ...
  sdreport_vector sdrLnRecMMB(styr,endyr-6)
  sdreport_vector sdrLnRec(styr,endyr-6)
  sdreport_vector sdrRec(styr,endyr-6)
  sdreport_vector sdrMMB(styr,endyr-6)
...
//  likeprof_number mature2
  ...
  vector obs_catchf_biom(styr,endyr)
...
PRELIMINARY_CALCS_SECTION
 ...
 for (i=1;i<=nobs_fix;i++)
 {
   s_fix(1,i) = 0.1*obs_s_fix(1,i);
   if (s_fix(1,i)>nsamples_fix(1,i)) s_fix(1,i) = nsamples_fix(1,i);
   s_fix(2,i) = 0.1*obs_s_fix(2,i);
   if (s_fix(2,i)>nsamples_fix(2,i)) s_fix(2,i) = nsamples_fix(2,i);
 }
 for (i=1;i<=nobs_tc;i++)
 {
...
//fixed gear bycatch biomass
 obs_catchf_biom = 0.0;
 for (i=1; i<= nobs_fix; i++)
 {
   obs_catchf_biom(yrs_fix(i))=(obs_p_fix(1,i)*catch_fix(yrs_fix(i)))*wt(1)+(obs_p_fix(2,i)*catch_fix(yrs_fix(i)))*wt(2);
 }
...
//   cout<<"end of female growth 1 " <<endl;
//females...83-93
   tt3.initialize();
   ttt.initialize();
   betal = mfexp(log_betal(1));
...
// trawl fishery selectivity
     ...
     sel_tem3(j)=1./(1.+mfexp(-1.*fish_disc_slope_fi*(length_bins(j)-mfexp(log_fish_disc_sel50_fi))));
...
//     sel_tcm2(j)=1./(1.+mfexp(-1.*fish_tc_slope_m2*(length_bins(j)-mfexp(log_fish_tc_sel50_m2))));
    ...
    maxsel_fish = max(sel_tem3);
    sel_tem3 = sel_tem3/maxsel_fish;
...
//     cout<<sel(2005)<<endl;
     ...
     sel_fix_f(iy) = sel_tem3;
     sel_fix_m(iy) = sel_tem3;
  }
...
//     sel_trawl_f(yrs_trawl(iy)) = sel_trawl_f(yrs_trawl(iy))/maxsel_fish;
 ...
 qm2 = mfexp(mfexp(qm2y))/(mfexp(mfexp(qm2y))+1.0);
 qf2 = qm2;
...
//  cout<<" ratio of females to males fishing mortality:"<<" "<<t0<<endl;
 ...
 for (i=yrs_trawl(1); i<=endyr; i++) fmortt(i)=mfexp(log_avg_fmortt+fmortt_dev(i));
   ...
   fmortf = 0.0;     //no fixed gear bycatch before 2009.
 for (i=yrs_fix(1); i<=endyr; i++) fmortf(i)=mfexp(log_avg_fmortf+fmortf_dev(i));
...
//have discard mort for females and males, fishing F for males only
   ...
   Fdiscfi(1,i)=sel_fix_f(i)*fmortf(i);
   Fdiscfi(2,i)=sel_fix_m(i)*fmortf(i);
   ...
   Ftot(1,i)=Fdiscf(i) + Fdisct(1,i) + Fdiscfi(1,i);
   Ftot(2,i)= F(i) + Fdisct(2,i) + Fdiscfi(2,i);
...
// cout<<lbio<<endl;
  ...
  if (active(qm2y)) qm2p = qm2;
  for (i=styr; i<= endyr-6; i++)
  {
     sdrRec(i) = (mfexp(mean_log_rec+rec_dev(1,i+6))+mfexp(mean_log_rec+rec_dev(2,i+6)))/1000000.0;
     sdrLnRec(i) = log(sdrRec(i));
     sdrMMB(i) = mbio215(i)/1000.0;
...
// catch(fishing season weighted by the catch), then rest of nat mort and growth.
    ...
    totn_fix(1,i)= na_fishtime(1,i)*sel_fix_f(i);                   //used for estimating proportion of trawl bycatch by length only.
    totn_fix(2,i)= na_fishtime(2,i)*sel_fix_m(i);
    catch_fi(i) = totn_fix(1,i)+totn_fix(2,i);
    if (catch_fi(i) < 0.000001) catch_fi(i) = 0.000001;
    for (j = 1; j<= nlenm; j++)
    {
...
//  cout<<na_fishtime(2,i)<<endl;
   ...
   pred_catch_fix(i)= na_fishtime(1,i)*elem_prod(1.0-exp(-1.0*Fdiscfi(1,i)),wt(1))+
                        na_fishtime(2,i)*elem_prod(1.0-exp(-1.0*Fdiscfi(2,i)),wt(2));
   ...
   pred_p_fix(1,i)=elem_prod(sel_fix_f(i),na_fishtime(1,i))/catch_fi(i);
   pred_p_fix(2,i)=elem_prod(sel_fix_m(i),na_fishtime(2,i))/catch_fi(i);
 }
 for (i=1;i<=nobs_tc;i++)
// for (i=1;i<=4;i++)
 {
...
// cout<<"start reference points"<<endl;
 ...
 {
   ref_F = 0.01*j-0.01;
   ref_Ftot = ref_sel_trawl*ref_Ft + ref_sel_fix*ref_Ff + ref_sel_fit*ref_F+ref_sel_ret*ref_F;
...
//     ref_mbio215(i) = (elem_prod(mfexp(-(0.694-tc_cm(endyr))*M(2,endyr))*mat2,elem_prod(mfexp(-(tc_cm(endyr)-cm(endyr))*M(2,endyr))*elem_prod(tem_S,mfexp(-cm(endyr)*M(2,endyr))*ref_na(i)),tem_Stc)))*wt(2);
    ...
    ref_catch_fix(i)= ref_na_fishtime(i)*elem_prod(1.0-exp(-1.0*ref_sel_fix*ref_Ff),wt(2));
  ...
  for (j = 1; j<= 101; j++)
 {
  ...
  ref_f = ref_f/2.0;
  ofl_f = f35;
  ref_Ftot = ref_sel_trawl*ref_Ft + ref_sel_fix*ref_Ff +  ref_sel_fit*ofl_f+ref_sel_ret*ofl_f;
  ref_S = mfexp(-1.0*ref_Ftot);
  ...
  if (last_mmb < eb35)
  {
   ...
   ref_catch_fix(1)= na_fishtime(2,endyr)*elem_prod(1.0-exp(-1.0*ref_sel_fix*ref_Ff),wt(2));
   ref_catch_fix(1) += na_fishtime(1,endyr)*elem_prod(1.0-exp(-1.0*ref_sel_fix*ref_Ff),wt(1));
   ref_catch_f = na_fishtime(1,endyr)*elem_prod(1.0-exp(-1.0*ref_sel_f*ref_f),wt(1));
   ref_catch(1) += ref_catch_f + ref_catch_trawl(1) + ref_catch_fix(1);
   ofl_catch = ref_catch(1);


  ofstream report1("refp75162d.out");
...
// cout<<vu<<endl;
  ...
  ofstream report2("prj75162d.out");
 for(int kk=1; kk<=3; kk++)         //F=0, F40%, & F35%
 {
   prj_Ft = ref_sel_trawl*ref_Ft + ref_sel_fix*ref_Ff;
...
//   t1 = pred_p_trawl(1,ij,j)*(1.0-pred_p_trawl(1,ij,j))+0.1/double(nlenf);
    ...
    len_like(5) += (-0.5)*log(6.29*t1)-log(sqrt(1.0/s_trawl(1,i)))+log(mfexp(-1.*square(obs_p_trawl(1,i,j)-pred_p_trawl(1,ij,j))*s_trawl(1,i)/(2.0*t1))+0.01);
   }
   for (j=1; j<=nlenm; j++)
   {
...
// len_like(5)-=nsamples_trawl(k,i)*(1e-6+obs_p_trawl(k,i,j))*log(pred_p_trawl(k,ij,j)+1e-6);
 ...
 for (i=1; i<= nobs_fix; i++)
 {
   ...
   {
//   t1 = pred_p_trawl(1,ij,j)*(1.0-pred_p_trawl(1,ij,j))+0.1/double(nlenf);
     t1 = obs_p_fix(1,i,j)*(1.0-obs_p_fix(1,i,j))+0.1/double(nlenf);
    len_like(9) += (-0.5)*log(6.29*t1)-log(sqrt(1.0/s_fix(1,i)))+log(mfexp(-1.*square(obs_p_fix(1,i,j)-pred_p_fix(1,ij,j))*s_fix(1,i)/(2.0*t1))+0.01);
   }
   for (j=1; j<=nlenm; j++)
   {
//   t1 = pred_p_fix(2,ij,j)*(1.0-pred_p_fix(2,ij,j))+0.1/double(nlenm);
     t1 = obs_p_fix(2,i,j)*(1.0-obs_p_fix(2,i,j))+0.1/double(nlenm);
     len_like(9) += (-0.5)*log(6.29*t1)-log(sqrt(1.0/s_fix(2,i)))+log(mfexp(-1.*square(obs_p_fix(2,i,j)-pred_p_fix(2,ij,j))*s_fix(2,i)/(2.0*t1))+0.01);
// len_like(5)-=nsamples_trawl(k,i)*(1e-6+obs_p_trawl(k,i,j))*log(pred_p_trawl(k,ij,j)+1e-6);
   }
 }
// cout<<"female discard length likelihood: "<<len_like(3)<<endl;
// cout<<"trawl discard length likelihood: "<<len_like(5)<<endl;
// cout<<"fixed gear discard length likelihood: "<<len_like(9)<<endl;
//survey likelihood
 for (i=1; i<=nobs_srv1; i++)
 {
   ii=yrs_srv1(i);
...
// cout<<"survey length likelihood: "<<len_like(4)<<endl;
 ...
 f-=like_wght(7)*len_like(9);     // Fixed gear
...
// q_like = pow((qm2-sur_q(1)),2)/(2.0*q_tem1)+y_bsfrf*pow((qm2-q_bsfrf(1)),2)/(2.0*q_tem3);
 if (active(qm2y))
 {
    q_like = 0.0;   //rk7516c.tpl
      //  q_like = pow((qm2-sur_q(1)),2)/(2.0*q_tem1);        //rk7516b.tpl
...
//survey abundance
 ...
 for(i=yrs_fix(1);i<=endyr;i++)
 {
    bio_04(i)=pred_catch_fix(i);
 }
 for(i=yrs_fish_discf(1);i<=endyr;i++)
 {
 ...
 catch_likefi = norm2(log(obs_catchf_biom+1.0)-log(bio_04+1.0));
// catch_liket = norm2(log(obs_catcht_biom+1.0)-log(pred_catch_trawl+1.0));
// catch_likefi = norm2(log(obs_catchf_biom+1.0)-log(pred_catch_fix+1.0));
...
// cout<<"rec_like"<<" len_like ret"<<" len_like m dis"<<" len_like f"<<" len_like survey"<<" len_like trawl"<<" len_like fix"<<" TC_like freq."<<endl;
// cout<<rec_like<<"    "<< len_like(1)<<"   "<<len_like(2)<<"   "<<len_like(3)<<"   "<<len_like(4)<< "   "<<len_like(5)<<"   "<<len_like(9)<<"   "<<len_like(6)+len_like(7)<<"  "<<endl;
...
//report<<" length - length transition matrix Males :'67.5-162.5'"<< endl;
  ...
  report << "selectivity trawl males+females:'67.5-162.5'"<< endl;
  report <<sel_trawl_m(styr)<<endl;
  report << "selectivity fixed males+females:'67.5-162.5'"<< endl;
  report <<sel_fix_m(styr)<<endl;
...
//  report << sel_srv1(2) << endl;
  ...
  report <<"likelihood : 'rec_like','len_like_ret','len_like_discmale','len_like_fem','len_like_surv','len_like_trawl','len_like_Tanner F then M','len_like_bsfrf','len_like_fix','catch ret','catch_disc-males','catch fem','catch trawl','catch fixed','Tanner bio','surv_like','bsfrf_bio', 'q_like','total likelihood','sexr_like','change_sel_like'"<<endl;
  report <<rec_like<<"  "<<-1.0*len_like<<" "<<catch_like2*like_wght(6)<<" "<<catch_like1*bycatch_w<<" "<<catch_likef*bycatch_w<<" "<<catch_liket*0.5*bycatch_w<<" "<<catch_likefi*0.5*bycatch_w<<" "<<(catch_like_tcf+catch_like_tcm)*bycatch_w<<" "<<surv_like<<" "<<bsfrf_like<<" "<<q_like<<" "<<f<<"  "<<sexr_like*like_wght_sexr<<"  "<<sel_like_50m<<endl;
...
//    }
 ...
 report << "Observed Length Prop fixed bycatch females: 'year','67.5-162.5'" << endl;
    for (i=1; i<=nobs_fix; i++)
    {
       report << yrs_fix(i) <<" " <<obs_p_fix(1,i) << endl;
    }
 report << "Predicted length prop fixed bycatch females: 'year','67.5-162.5'" << endl;
    for (i=1; i<=nobs_fix; i++)
    {
      ii=yrs_fix(i);
      report << ii << " " << pred_p_fix(1,ii) << endl;
    }
 report << "Observed Length Prop fixed bycatch males: 'year','67.5-162.5'" << endl;
    for (i=1; i<=nobs_fix; i++)
    {
       report << yrs_fix(i) <<" " <<obs_p_fix(2,i) << endl;
    }
 report << "Predicted length prop fixed bycatch males: 'year','67.5-162.5'" << endl;
    for (i=1; i<=nobs_fix; i++)
    {
      ii=yrs_fix(i);
      report << ii << " " << pred_p_fix(2,ii) << endl;
    }
...
//    }
 ...
 report << "Estimated numbers of old shell male crab by length: 'year','67.5-162.5'"<< endl;
  ...
  report << sel_fit << endl;
  report <<"likelihood weights: 'retained length','total catch length','female catch','survey length','survey biomass','catch biomass','trawl/fix length'"<<endl;
  ...
  report << "predicted fixed gear discarded biomass : seq(1968,2008)" << endl;
  report << pred_catch_fix<<endl;
  ...
  report << fmortt<<endl;
  report << "Fixed gear fishing mortality" << endl;
  report << fmortf<<endl;
...
//  report << sel_tcm2 << endl;
  ...
  report3 << "nobs_fix" << endl << nobs_fix << endl;            //number of years of fixed gear bycatch length data
  report3 << "yrs_fix" << endl << yrs_fix << endl;          //years when have fixed gear bycatch data
  ...
  report3 << "obs_catchf_biom" <<endl << obs_catchf_biom <<endl;
  report3 << "pred_catchf_biom" <<endl << bio_04<<endl;
  ...
  report3 << "sel_fixf" << endl <<sel_fix_f(styr)<<endl;
  report3 << "sel_fixm" << endl <<sel_fix_m(styr)<<endl;
  ...
  report3 << "fish_mort_fix" << endl << fmortf << endl;
  ...
  report3 << "obs_p_fixf" << endl;
  for (i=1; i<=nobs_fix; i++)
  {
      report3 <<obs_p_fix(1,i) << endl;   //Observed Length Prop fix bycatch females
  }
  report3 << "pred_p_fixf" << endl;
  for (i=1; i<=nobs_fix; i++)
  {
      ii=yrs_fix(i);
      report3 << pred_p_fix(1,ii) << endl;    //Predicted length prop fixed bycatch females
  }
  report3 << "obs_p_fixm" << endl;
  for (i=1; i<=nobs_fix; i++)
  {
      report3 <<obs_p_fix(2,i) << endl;   //Observed Length Prop fixed bycatch males
  }
  report3 << "pred_p_fixm" << endl;
  for (i=1; i<=nobs_fix; i++)
  {
      ii=yrs_fix(i);
      report3 << pred_p_fix(2,ii) << endl;    //Predicted length prop fix bycatch males
  }
