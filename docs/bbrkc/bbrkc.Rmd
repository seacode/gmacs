...
for (m in 1:length(like))
{
  ...
  {
    nloglike <- merge(nloglike, nll, by.x = 1, by.y = 1, all = TRUE)
  }
}
rownames(nloglike) <- c("Catch","Abundance","Size composition","Recruitment deviations","Growth increment")
colnames(nloglike) <- c("OneSex","TwoSex")
tab <- xtable(nloglike, digits = c(2), caption = "Likelihoods in log-space.")
...
tab <- xtable(nlogPenalty, digits = c(2), caption = "Penalties in log-space.")
print(tab, caption.placement = "top")
```
