...
h. The base natural mortality is constant over shell condition and size and
was estimated assuming a maximum age of 25 and applying the 1% rule (Zheng
2005).

ii. Survey and fisheries selectivities are a function of size and were
...
iii. Growth is a function of size and did not change over time for males. For
females, three growth increments per molt as a function of size were estimated
...
iv. Molting probabilities are an inverse logistic function of size for
...
ix. Measurement errors were assumed to be normally distributed for size
...
For the size-weight relationships, Jie's data file `rk7513s1.dat` has
information on the weight-at-size parameters for BBRKC. He suggests we use the
'new' parameters listed (see line 339 onwards): these parameters were estimated
by NMFS.
 
Fecundity-at-size is a little more complicated: This information was provided
by Jie:

From Jie: Fecundity-at-size depends on clutch fullness, which changes from
...
looks like the male mean weight by size with the “old" parameters”. If GMACS
needs fecundity, maybe just input mean weight by size of mature females, or
mature males (please use “new” parameters). As to the maturity by size, right
...
for males. In the future, I plan to estimate maturity by size for females over
...
The mean weight at size used in the two models differs little for males
(Figure \ref{fig:length-weight}). However, the pattern is very different for
females. This difference is due to... 

```{r length_weight, fig.cap = "Relationship between carapace length (mm) and weight (kg) by sex.\\label{fig:length-weight}"}
...
```{r natural_mortality, fig.cap = "Time-varying natural mortality ($M_t$).\\label{fig:Mt}"}
...
```{r spawning_stock_biomass, fig.cap = "Mature male biomass.\\label{fig:ssb}"}
plot_ssb(M)
```
...
```{r catch, fig.cap = "Retained catch and bycatch mortality biomasses for BBRKC from 1975 to 2013.\\label{fig:catch}"}
