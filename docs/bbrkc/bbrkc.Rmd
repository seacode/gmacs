...
which test the efficacy of Gmacs and illustrate how alternative models can be specified.
...
New features that under development include:
...
```{r survey_biomass, fig.cap = "Model fits to sex-specific NMFS trawl survey biomass (tonnes) from 1975 to 2014. The error bars represent plus and minus 2 standard deviations.\\label{fig:survey_biomass}"}
...
```{r fit_to_catch, fig.cap = "Observed and predicted catch (tonnes) by gear type for the two Gmacs models. The OneSex model includes catch data from 1953 to 2013. The TwoSex model includes catch data from 1975 to 2013.\\label{fig:fit_to_catch}"}
...
```{r , sc_NMFS_nm, fig.cap = "Observed and model estimated size-frequencies of new shell male BBRKC by year in the NMFS trawl fishery.\\label{fig:sc_NMFS_nm}"}
plot_size_comps(A, 6)
```

```{r , sc_NMFS_om, fig.cap = "Observed and model estimated size-frequencies of old shell male BBRKC by year in the NMFS trawl fishery.\\label{fig:sc_NMFS_om}"}
plot_size_comps(A, 7)
```

```{r , sc_NMFS_f, fig.cap = "Observed and model estimated size-frequencies of female BBRKC by year in the NMFS trawl fishery.\\label{fig:sc_NMFS_f}"}
plot_size_comps(A, 8)
```

```{r , sc_BSFRF_no, fig.cap = "Observed and model estimated size-frequencies of both male and female BBRKC by year in the BSFRF trawl surveys.\\label{fig:sc_BSFRF_no}"}
plot_size_comps(A, 9)
```
