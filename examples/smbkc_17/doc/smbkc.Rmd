...
mod_names <- c("2016 Model","2017 BTS","Reference","VAST","Fit surveys","Francis weights") #,"Gmacs M","Gmacs Francis","fit surveys")
names(M)  <- mod_names
```

### b. Fitting to the alternative NMFS bottom-trawl survey index

Results comparing model fits between the "VAST" spatio-temporal index and the reference case show different time-series of data
and a different model fit (Figure \ref{fig:surv3}). The effect on spawning biomass suggests was consistently higher estimates
 since 1990 compared to the reference model (Figure  \ref{fig:ssb2}).
...
stage composition residuals for trawl survey, pot survey, and commercial observer data are shown for the reference model,
  Figures \ref{fig:sc_res_base}
through  \ref{fig:sc_res_disc}.
...
biomasses for models are shown in Figures \ref{fig:init_N} and \ref{fig:mmb}.
...
Mature male biomass (Figure \ref{fig:mmb}), the **fit surveys** sensitivity stands out as being
quite different to the other models. This scenario results in a lower MMB from the mid-1980s
...
Parameter <- c("Natural mortality deviation in 1998/99 ($\\delta^M_{1998})$",
              ...
              "$F_\\text{OFL}$","OFL")
df <- data.frame(Parameter, Estimate, SD)
tab <- xtable(df, caption = "Model parameter estimates, selected derived quantities, and their standard deviations (SD) for the \"Francis weigths\" model.", label = "tab:est_pars_fit_survey", digits = 3)
print(tab, caption.placement = "top", include.rownames = FALSE, sanitize.text.function = function(x){x})
```
...
tab <- xtable(df, caption = "Comparisons of negative log-likelihood values for the four model scenarios. It is important to note that some of these model runs are directly comparable as the {\\bf Francis} model implements the Francis iterative reweighting method (Francis 2011).", label = "tab:likelihood_components")
print(tab, caption.placement = "top", include.rownames = FALSE, sanitize.text.function = function(x){x})
```
...
```{r sc_pot_res_selex, fig.cap = "Bubble plots of residuals by stage and year for the directed pot fishery size composition data for St. Mathew Island blue king crab (SMBKC) in the reference model.\\label{fig:sc_res_base}"}
fn <- paste0(.MODELDIR[3], "gmacs"); MM <- lapply(fn, read_admb); names(MM) <- c("SMBKC")
plot_size_comps_res(MM)
```

```{r sc_pot_discarded_res_selex, fig.cap = "Bubble plots of residuals by stage and year for the NMFS trawl survey size composition data for St. Mathew Island blue king crab (SMBKC) in the reference model.\\label{fig:sc_res_disc}"}
fn <- paste0(.MODELDIR[3], "gmacs"); MM <- lapply(fn, read_admb); names(MM) <- c("SMBKC")
plot_size_comps_res(MM)
```
