...
fourth lowest since 2000) with a biomass of 90mm and larger male crab of just under 1.8 kt (~31% of the long term mean; 
...
5. **Management performance**: In this assessment estimated total male catch is the sum of
...
as these have been the only sources of non-negligible fishing mortality to
consider. Based on the baseline model for SMBKC, the stock was above the minimum stock-size threshold (MSST) in 2016/17 and is 
hence not overfished.
Overfishing did not occur in this year as the directed fishery was closed (Tables \ref{tab:status} and \ref{tab:status_pounds}).
Nonetheless, the low survey values and paucity of crabs in the region, as indicated by the surveys, remains a concern.
...
Details of this implementation is provided in Appendix A.

## Changes in Assessment Results

Both surveys indicate a decline over the past few years. The "reference" model is that selected for use in 2016. 
The addition of new data introduced this year area presented sequentially. Two alternative models are presented for
sensitivity. One involves a re-analysis of the NMFS trawl survey data using a spatio-temporal Delta-GLMM approach (VAST model, Thorson 2016)
and the other simply adds emphasis to fit the recent survey data better. In all cases, the
model tends to moderate the declines observered in the surveys (except in the "Fit survey" case).
...
t (1.202 million pounds) in 1977, and harvests peaked in 1983 when 164
vessels landed 4288 t (9.454 million pounds) (Fitch et al. 2012; Table
...
These data suggest a reduction in the bycatch of females, which may be attributable
...
in the eastern Bering Sea snow crab fishery, but in recent years it has generally been negligible. 
The St. Matthew Island golden king crab fishery, the third commercial crab fishery to have taken place in the area, 
typically occurred in areas with depths
...
tab <- xtable(df, caption = "Groundfish SMBKC male bycatch biomass (t) estimates. Trawl includes pelagic trawl and non-pelagic trawl types. Source: J. Zheng, ADF\\&G, and author estimates based on data from R. Foy, NMFS. AKRO estimates used after 2008/09.", label = "tab:smbkc_groundfish_bycatch", digits = c(0,0,3,3))
...
eastern Bering Sea trawl survey (1978-2017; Table \ref{tab:stage_cpue_nmfs}); results from the triennial ADF&G SMBKC pot
survey (every third year during 1995-2013, then 2015-2017) pot survey (Table \ref{tab:stage_cpue});
...
assessment,  effort to match the 2015 SMBKC stock assessment model to bridge a framework which provided greater
...
reference model being the same configuration as last year, three sensitivities are considered, one with a 
different treatment of NMFS bottom trawl survey (BTS) data using a geo-spatial model, another which weights
the survey data more heavily, and a third which weights the size composition data according to Francis' 2011 approach.
In addition to these sensitivities, we also evaluated the impact of adding
new data to the reference model. In summary, the following lists the models presented and the naming convention 
used: 
...
3. **VAST**: applies a geo-spatial delta-GLMM model to the BTS data which provides a different BTS index. See appendix B for 
...
3. **Fit survey**: an exploratory scenario that the same as above except the NMFS trawl survey is up-weighted by
  $\lambda^\text{NMFS}=$ `r M[[5]]$cpue_lambda[1]` and the ADF&G pot survey is up-weighted by $\lambda^\text{ADFG}=$
   `r M[[5]]$cpue_lambda[2]`. 
...
5. **Gmacs Francis**: 
is similar to the scenario above except that it also uses the Francis iterative re-weighting
method (Francis 2011), to re-weight the size-composition data relative to the abundance indices. The trawl survey and
...
\ref{fig:surv1} and \ref{fig:surv2} with recruitment and spawning biomass shown in figures \ref{fig:rec1} and \ref{fig:ssb1}, respectively. 
 ...
 in the 2016 assessment, especially with the addition of the pot survey.

```{r surv1, fig.cap = "Comparisons of fits to area-swept estimates of total (>90mm) male survey biomass (t). Error bars are plus and minus 2 standard deviations.\\label{fig:surv1}"} 
plot_cpue(M[1:3], "NMFS Trawl", ylab = "Survey biomass (t)") 
```


```{r surv2, fig.cap = "Comparisons of fits to CPUE from the ADF&G pot surveys. Error bars are plus and minus 2 standard deviations.\\label{fig:surv2}"} 
...
### b. Fitting to the alternative NMFS bottom-trawl survey index

Results comparing model fits between the "VAST" spatio-temporal index and the reference case are shown in Figure 

\ref{fig:surv3} and \ref{fig:surv4} with recruitment and spawning biomass shown in figures \ref{fig:rec1} and \ref{fig:ssb1}, respectively. 
 The fits to survey CPUEs and spawning biomass show that the addition of new data result in more of a decline than
 in the 2016 assessment, especially with the addition of the pot survey.

```{r surv3, fig.cap = "Comparisons of fits to area-swept estimates of total (>90mm) male survey biomass (t). Error bars are plus and minus 2 standard deviations.\\label{fig:surv3}"} 
plot_cpue(M[3], "NMFS Trawl", ylab = "Survey biomass (t)") 
```

```{r surv4, fig.cap = "Comparisons of fits to area-swept estimates of total (>90mm) male survey biomass (t). Error bars are plus and minus 2 standard deviations.\\label{fig:surv4}"} 
plot_cpue(M[4], "NMFS Trawl", ylab = "Survey biomass (t)") 
```
...
is acceptable at 1.xx44 in the reference model, and improves to 1.xx1 in the **hhh base** scenario. In the
model the SDNR of the trawl survey is slightly worse at 1.59, and is much worse in the exploratory **Gmacs
force** scenario at 2.16. The SDNRs for the pot surveys show much the same pattern between each of the scenarios, but
are much higher values (ranging from 3.95 to 5.19). 
These values are very high, and whilst they can be improved by down-
...
0.78 to 1.30 (except for in the **fit surveys** scenario where the weights were a little high). The SDNRs for the
...
in the **Gmacs Francis**, and **fit surveys** scenarios). Instead, the Francis size composition weights were used
(Francis 2011).
...
**Gmacs M**, **Gmacs Francis**, and **fit surveys** scenarios in Figures \ref{fig:sc_res_base},
...
not attempt to estimate natural mortality in 1998/99 (**Gmacs M**, **Gmacs Francis**, and **fit surveys**) predict lower
...
NMFS survey biomass and ADF&G pot survey CPUE (**fit surveys**) provides a better fit to the survey biomass during the
...
Estimated standard deviations of parameters and selected management measures for the four models are summarized
in Tables \ref{tab:est_pars_ref}, \ref{tab:est_pars_vast}, \ref{tab:est_pars_force}, and
\ref{tab:est_pars_Francis}. Probabilities for mature male biomass and OFL in 2017 are illustrated in Section F.
...
Looking at the plot of mature male biomass (Figure \ref{fig:mmb}), the **fit surveys** scenario stands out as being
...
Parameter <- c("$\\log (\\bar{R})$","$\\log (n^0_1)$","$\\log (n^0_2)$","$\\log (n^0_3)$","ADF\\&G pot survey catchability ($q \\times 1000$)",
               ...
               "$F_\\text{OFL}$","OFL")
df <- data.frame(Parameter, Estimate, SD)
tab <- xtable(df, caption = "Model parameter estimates, selected derived quantities, and their standard deviations (SD) for the {\\bf fit surveys} model.", label = "tab:est_pars_force", digits = 3)
print(tab, caption.placement = "top", include.rownames = FALSE, sanitize.text.function = function(x){x})
```
...
Estimate <- NULL
Model <- NULL
Mname <- c("Ref","VAST","Fit Survey","Francis")
for (ii in 3:6)
{
    x <- M[[ii]]$fit
    i <- c(grep("m_dev", x$names)[1],
...
tab <- xtable(df, caption = "Comparisons of model parameter estimates for the four model scenarios.", label = "tab:est_pars_all", digits = 3)
...
df <- data.frame(rownames(df), df, row.names = NULL)
names(df) <- c("Component","Match","Base","M","Francis","Force")
tab <- xtable(df, caption = "Comparisons of negative log-likelihood values for the four model scenarios. It is important to note that not all of these model runs are directly comparable as the {\\bf Gmacs Francis} and {\\bf fit surveys} models implement the Francis iterative reweighting method (Francis 2011).", label = "tab:likelihood_components")
print(tab, caption.placement = "top", include.rownames = FALSE, sanitize.text.function = function(x){x})
```
...
df <- data.frame(as.integer(A$mod_yrs), A$N_len[,1], A$N_len[,2], A$N_len[,3], c(A$ssb))
names(df) <- c("Year","$n_1$","$n_2$","$n_3$","MMB")
tab <- xtable(df, digits = 0, caption = "Population abundances ($\\boldsymbol{n}$) by crab stage in numbers of crab at the time of the survey (1 July, season 1) and mature male biomass (MMB) in tons on 15 February for the {\\bf Gmacs Francis} model.", label = "tab:pop_abundance_vast")
print(tab, caption.placement = "top", include.rownames = FALSE, sanitize.text.function=function(x){x})
```
...
```{r selectivity, fig.cap = "Comparisons of the estimated stage-1 and stage-2 selectivities for each of the different model scenarios (the stage-3 selectivities are all fixed at 1). Estimated selectivities are shown for the directed pot fishery, the trawl bycatch fishery, the fixed bycatch fishery, the NMFS trawl survey, and the ADF&G pot survey. Two selectivity periods are estimated in the directed pot fishery, from 1978-2008 and 2009-2017.\\label{fig:selectivity}", fig.height = 15}
plot_selectivity(M[c(1,3)], ncol = 5)
```
<!-- 
```{r trawl_survey_biomass, fig.cap = "Comparisons of area-swept estimates of total male survey biomass (tons) and model predictions for the 2015 model and each of the Gmacs model scenarios. The error bars are plus and minus 2 standard deviations.\\label{fig:trawl_survey_biomass}"} plot_cpue(M, "NMFS
Trawl", ylab = "Survey biomass (tons)") 
```
...
```{r sc_trawl_discarded_res, fig.cap = "Bubble plots of residuals by stage and year for the ADF&G pot survey size composition data for St. Mathew Island blue king crab (SMBKC) in the **fit surveys** model.\\label{fig:sc_res_force}"}
fn <- paste0(.MODELDIR[3], "gmacs"); MM <- lapply(fn, read_admb); names(MM) <- c("SMBKC")
plot_size_comps_res(MM)
```
