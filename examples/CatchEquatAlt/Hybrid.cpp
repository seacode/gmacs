    ...
    for (Isex=1;Isex<=Nsex;Isex++)
     for (Iage=0;Iage<=Nage;Iage++)
      for (Iplatoon=1;Iplatoon<=Nplatoon;Iplatoon++)
       vbio += sum(elem_prod(N(Year,Isex,Iage,Iplatoon),selretwghtUse(f,Isex,Iage)));     
    ...
    for (Iplatoon=1;Iplatoon<=Nplatoon;Iplatoon++)
     for (Iage=0;Iage<=Nage;Iage++)
      for (Isize=1;Isize<=NsizeDym;Isize++)
       {
        Z_rate(Isex,Iplatoon,Iage,Isize) = NatM(Isex,Year);
        for (f=1;f<=NfishFleet;f++)
         Z_rate(Isex,Iplatoon,Iage,Isize) += selexS(f,Isex,Iage,Isize) * Hrate(f,Year);
      ...
      if (Catch(f,Year) > 0)
       {
        for (Isex=1;Isex<=Nsex;Isex++)
         for (Iage=0;Iage<=Nage;Iage++)
          for (Iplatoon=1;Iplatoon<=Nplatoon;Iplatoon++)
           for (Isize=1;Isize<=NsizeDym;Isize++)
            Z_adjuster2 += Hrate(f,Year)*N(Year,Isex,Iage,Iplatoon,Isize)*selretwghtUse(f,Isex,Iage,Isize)*Z_rate2(Isex,Iplatoon,Iage,Isize);
       }
     ...
     for (Isex=1;Isex<=Nsex;Isex++)
      for (Iplatoon=1;Iplatoon<=Nplatoon;Iplatoon++)
       for (Iage=0;Iage<=Nage;Iage++)
        for (Isize=1;Isize<=NsizeDym;Isize++)
         {
          Z_rate(Isex,Iplatoon,Iage,Isize)  = NatM(Isex,Year) + Z_adjuster*(Z_rate(Isex,Iplatoon,Iage,Isize)-NatM(Isex,Year));
          Z_rate2(Isex,Iplatoon,Iage,Isize) = (1-mfexp(-Z_rate(Isex,Iplatoon,Iage,Isize)))/Z_rate(Isex,Iplatoon,Iage,Isize);  
         }
      ...
      if (Catch(f,Year) > 0)
       {
        ...
        for (Isex=1;Isex<=Nsex;Isex++)
         for (Iage=0;Iage<=Nage;Iage++)
          for (Iplatoon=1;Iplatoon<=Nplatoon;Iplatoon++)
           for (Isize=1;Isize<=NsizeDym;Isize++)
            Z_adjuster2 += N(Year,Isex,Iage,Iplatoon,Isize)*selretwghtUse(f,Isex,Iage,Isize)*Z_rate2(Isex,Iplatoon,Iage,Isize);
     ...
     for (Iage=0;Iage<=Nage;Iage++)
      for (Iplatoon=1;Iplatoon<=Nplatoon;Iplatoon++)
       if (Model_Type == AGEMODEL)
        ...
        {
         Surv = mfexp(-1*SurveyTime(f)*Z_rate(Isex,Iplatoon,Iage,Isex));
         for (Isize=1;Isize<=NsizeDym;Isize++)
