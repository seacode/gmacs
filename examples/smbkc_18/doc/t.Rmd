...
```{r surv1, fig.cap = "Fits to NMFS area-swept trawl estimates of total (90+ mm) male survey biomass with the addition of new data. Error bars are plus and minus 2 standard deviations.\\label{fig:surv1}"} 
...
```{r surv3, fig.cap = "Comparisons of fits to area-swept estimates of total (90+ mm) male survey biomass (t) for the standard design-based estimate and for estimates derived from the VAST spatio-temporal model of Thorson and Barnett (2017). Error bars are plus and minus 2 standard deviations.\\label{fig:surv3}"} 
names(M[3])  <- "Reference"
...
names(M[3]) <- "Reference"
plot_cpue(M[c(3,5,6)],  "ADF&G Pot", ylab = "Pot survey CPUE (crab/potlift)")
```
