...
# The model specs
mod_names <- c("2017 Model","2018 BTS","Reference","VAST","Fit surveys") #,"Gmacs M","Gmacs Francis","fit surveys")
...
Estimate <- NULL
Model <- NULL
Mname <- c("2017","BTS","Ref","VAST","FitSurvey")
for (ii in mod_scen)
{
    x <- M[[ii]]$fit
    i <- c(grep("m_dev", x$names)[1],
...
df <- tidyr::spread(df, Model, Estimate) %>% dplyr::select(Parameter, Ref, VAST, FitSurvey)
