...
// --------------------------------------------------------------------------------------------------
  ...
  sdreport_vector sdrLnRecMMB(syr,nyr-7);             //these are for spawning per recruits. Six years of recruitment time lag.
  sdreport_vector sdrLnRec(syr,nyr-7);
  sdreport_vector sdrRec(syr,nyr-7);
  sdreport_vector sdrMMB(syr,nyr-7);
...
FUNCTION calc_sdreport
  ...
  for (int i=syr; i<= nyr-7; i++)
  // for (int i=syr+1; i<= nyr; i++)
  {
    sdrRec(i) = recruits(1,i+7);            //These are for spawning per recruits
    sdrLnRec(i) = log(sdrRec(i));
    sdrMMB(i) = mfexp(sd_log_ssb(i));
    sdrLnRecMMB(i) = log(sdrRec(i)/sdrMMB(i));
   // sdrRec(i) = recruits(1,i);         //These are for recruits
   // sdrLnRec(i) = log(sdrRec(i));
   // sdrMMB(i) = mfexp(sd_log_ssb(i));
